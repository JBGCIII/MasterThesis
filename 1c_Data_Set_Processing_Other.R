...
write_csv(gdp_sweden_log, "1_Processed_Data/2Data_Set_Columns/1_gdp_sweden_log.csv")
...
write_csv(consumption_processed, "1_Processed_Data/2Data_Set_Columns/2_consumption_categories_log.csv")
...
write_csv(household_indicators, "1_Processed_Data/2Data_Set_Columns/3-5_D_Savings_and_DTI.csv")
...
write_csv(policy_rate_quarterly, "1_Processed_Data/2Data_Set_Columns/9_policy_rate.csv")
...
write_csv(cpi_quarterly, "1_Processed_Data/2Data_Set_Columns/7_a_Inflation.csv")

# Foreign Inflation (Preserving HICP Index and Log Level)
ea_hicp_quarterly <- read_csv("0_Raw_Data/16_ECB_Euro_Area_HICP_monthly_raw.csv") %>%
  ...
  summarise(ea_hicp_index = mean(as.numeric(obsvalue), na.rm = TRUE), .groups = "drop") %>%
  mutate(
...
write_csv(ea_hicp_quarterly, "1_Processed_Data/2Data_Set_Columns/7_b_foreign_inflation_log.csv")

# ==============================================================================
# [8] Foreign GDP & [10] Real Exchange Rate
# ==============================================================================
...
write_csv(KIX_GDP_log_level, "1_Processed_Data/2Data_Set_Columns/8_C_kix_gdp_log.csv")

# Exchange Rates (Preserving Raw Nominal Index, Log Nominal, and Log Real)
kix_real_quarterly <- read_csv("1_Processed_Data/Data_Set_Columns/10_kix_exchange_rates.csv") %>%
  mutate(quarter = to_yyyyq(quarter)) %>%
  ...
  mutate(
    ...
    kix_real_log = kix_nominal_log + ea_hicp_log - cpi_log
  )

write_csv(kix_real_quarterly, "1_Processed_Data/2Data_Set_Columns/10_kix_exchange_rates.csv")
...
write_csv(unemployment_quarterly, "1_Processed_Data/2Data_Set_Columns/12_unemployment_rates.csv")
...
write_csv(brent_quarterly, "1_Processed_Data/2Data_Set_Columns/13_Oil_log.csv")
...
write_csv(fedfunds_quarterly, "1_Processed_Data/2Data_Set_Columns/14_fed_policy_rate.csv")
...
write_csv(consumer_confidence_quarterly, "1_Processed_Data/2Data_Set_Columns/15_consumer_confidence.csv")
...
write_csv(real_house_price_quarterly, "1_Processed_Data/2Data_Set_Columns/6_real_house_price.csv")
...
master_bsvar_data <- gdp_sweden_log %>%
  ...
  full_join(ea_hicp_quarterly, by = "quarter") %>%
...
inspection_data <- pre_inspection_data %>%
  transmute(
    ...
    ea_hicp_log = ea_hicp_log.x, # Euro Area Foreign Inflation
...
write_csv(
  inspection_data, 
  "1_Processed_Data/1_b_inspection_data.csv"
)
