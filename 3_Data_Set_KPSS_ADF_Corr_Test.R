...
var_name <- names(macro_data_inspection)[sapply(macro_data_inspection, is.numeric)]


data_set_adf_kpss <- read_csv(
    "1_Processed_Data/1_c_seasonally_adjusted_data.csv"
)

# Helper function to run ADF & KPSS tests on a single vector
check_stationarity <- function(macro_data_inspection_outliers, var_name, alpha = 0.05) {
  # Clean vector
  x <- na.omit(macro_data_inspection_outliers)
  
  # ----------------------------------------------------------------------------
  # 1. ADF Test (Null H0: Series has a unit root / Non-stationary)
  # ----------------------------------------------------------------------------
  adf_res <- ur.df(x, type = "drift", selectlags = "AIC")
  adf_stat <- adf_res@teststat[1, "tau2"]
  adf_crit_5pct <- adf_res@cval["tau2", "5pct"]
  
  # Reject H0 if ADF test stat < critical value
  adf_is_stationary <- adf_stat < adf_crit_5pct
  
  # ----------------------------------------------------------------------------
  # 2. KPSS Test (Null H0: Series is stationary)
  # ----------------------------------------------------------------------------
  kpss_res <- ur.kpss(x, type = "mu")
  kpss_stat <- kpss_res@teststat
  kpss_crit_5pct <- kpss_res@cval[1, "5pct"]
  
  # Reject H0 if KPSS test stat > critical value
  ...
  verdict <- case_when(
    ...
    adf_is_stationary & !kpss_is_stationary  ~ "Conflicting (ADF says I(0), KPSS says I(1))",
    !adf_is_stationary & kpss_is_stationary ~ "Conflicting (ADF says I(1), KPSS says I(0))"
  )
...
# ==============================================================================
# Run automatically across ALL numeric columns in your data frame
# ==============================================================================

# Select numeric columns (ignoring date/quarter)
numeric_cols <- names(data_set_adf_kpss)[sapply(data_set_adf_kpss, is.numeric)]

# Run loop and combine into one summary table
stationarity_summary <- map_dfr(numeric_cols, function(col) {
  check_stationarity(data_set_adf_kpss[[col]], var_name = col)
})
...
write_csv(stationarity_summary, "2_Data_Inspection/Stationarity_Summary.csv")
...
# 1. Read your seasonally adjusted dataset
data_set_adf_kpss <- read_csv("1_Processed_Data/1_c_seasonally_adjusted_data.csv")
...
df_stationary <- data_set_adf_kpss %>%
  mutate(
    ...
    d_ea_hicp_log                  = ea_hicp_log - lag(ea_hicp_log),
